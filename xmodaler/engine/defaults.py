...
class DefaultTrainer(TrainerBase):
    ...
    def build_hooks(self):
        ...
        ret = [
            ...
            hooks.ScheduledSampling(
                start_iter = cfg.SCHEDULED_SAMPLING.START_EPOCH * self.iters_per_epoch,    #wlx
                inc_every_iter = cfg.SCHEDULED_SAMPLING.INC_EVERY_EPOCH * self.iters_per_epoch,  #wlx
    ...
    def _write_metrics(
        ...
        if comm.is_main_process():
            ...
            total_losses_reduced = metrics_dict['Total Loss']

            # sum(metrics_dict.values())
            ...
            if 'Cap Loss' in metrics_dict:
                storage.put_scalar("{}cap_loss".format(prefix), metrics_dict['Cap Loss'])
            if 'RewardCriterion' in metrics_dict:
             storage.put_scalar("{}RewardCriterion".format(prefix), metrics_dict['RewardCriterion'])
            if 'Bbox Loss' in metrics_dict:
                storage.put_scalar("{}bbox_loss".format(prefix), metrics_dict['Bbox Loss'])
        ...
        with torch.no_grad():
            for data in tqdm.tqdm(test_data_loader):
                ...
                if cfg.INFERENCE.GENERATION_MODE == True:
                    ...
                    res = model(data)
                ids = np.stack(ids).reshape(-1).astype(int)
                outputs_cap = res[kfg.OUTPUT]
                outputs_bbox = res[kfg.BBOX_OUTPUT]
                # outputs_mp = res[kfg.MP_NUM_OUTPUT]
                for id, output_cap, output_bbox in zip(ids, outputs_cap, outputs_bbox):
                    results.append({cfg.INFERENCE.ID_KEY: int(id),
                                    cfg.INFERENCE.VALUE: output_cap,
                                    'bbox': output_bbox})
