...
class RLTrainer(DefaultTrainer):
    ...
    def run_step(self):
        ...
        bs_rewards,_ = self.scorer(bs_outputs_dict)

        self.model.train()
        data[kfg.DECODE_BY_SAMPLE] = True
        outputs_dict = self.model(data, use_beam_search=False, output_sents=False)
        rewards,num = self.scorer(outputs_dict)
        rewards = torch.from_numpy(rewards[kfg.REWARDS] - bs_rewards[kfg.REWARDS]).float().cuda()
        outputs_dict.update({ kfg.REWARDS: rewards })
        outputs_dict.update({ 'BBOX_NUM': num })
        outputs_dict.update({ 'G_TARGET_BBOX_IDS': bs_data['G_TARGET_BBOX_IDS'] })
        outputs_dict.update({ 'G_TARGET_CAP_IDS': bs_data['G_TARGET_CAP_IDS'] })
        ...
        losses_dict.update({'Total Loss': losses})
